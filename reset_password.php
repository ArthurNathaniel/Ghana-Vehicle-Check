...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM police WHERE email = ? AND reset_code = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $update_sql = "UPDATE police SET password = ?, reset_code = NULL WHERE email = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ss", $hashed_password, $email);
        $update_stmt->execute();

        $success_message = "Your password has been reset. You can now log in with your new password.";
        header("Location: police_login.php?reset=success");
        exit();
    } else {
